...
function SidebarProvider({
  ...
  // Internal state (controlled via openProp/setOpenProp when provided).
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Persist sidebar state across reloads via cookie.
...
function SidebarMenuButton({
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>{comp as React.ReactElement}</TooltipTrigger>
      {tooltipHidden ? null : (
...
function SidebarMenuSkeleton({
  ...
  // Random width between 50-90% for natural skeleton variation.
